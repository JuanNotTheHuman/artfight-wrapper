...
class ArtfightScrapper {
            ...
            await new Promise(r => {
                setTimeout(() => {
                    achv = Array.from(document.querySelectorAll(".row")[1].children)
                        ...
                        const src = grandChild?.src;
                        const title = grandChild?.dataset.originalTitle;
    ...
    async fetchUserCharacter(link) {
        ...
        let images = await page.evaluate(async () => {
            ...
            let imgs = cardBody && cardBody.children.item(0) ? Array.from(cardBody.children.item(0)?.children || []) : [];
            let elements = imgs.map(r => r.children.item(0));
    ...
    async #fetchSumbition(page, link) {
        ...
        const result = await page.evaluate(() => {
            const elements = Array.from(document.querySelectorAll("table.table")).map(r => {
            ...
            if (elements.length > 2) {
                ...
                if (level !== undefined) {
                    ...
                    if (level !== 0) {
                        ...
                        if (pdata) {
                            revenge.previous.link = pdata.children.item(1)?.children.item(0)?.href;
                            revenge.previous.title = pdata.children.item(1)?.children.item(0)?.children.item(0)?.getAttribute("data-original-title") || "";
                            revenge.previous.image = pdata.children.item(1)?.children.item(0)?.children.item(0)?.src;
        ...
        return new Submition(new SubmitionInformation(information[0] || '', information[1] || '', information[2] || '', information[3] || ''), new SubmitionStatistics(statistics[0] || '', statistics[1] || '', statistics[2] || '', statistics[3] || '', statistics[4] || '', statistics[5] || '', statistics[6] || ''), revenge, time ?? '', statistics[0]?.includes("Friendly Fire") || false, undefined, polished, []);
    }
    /**
    ...
    async fetchClientUserBookmarks(limit = 30) {
        if (limit <= 0)
            throw new Error("Limit cannot be negative or equal to 0");
                ...
                try {
                    ...
                    const response = await page.evaluate(() => {
                        return Array.from(document.querySelectorAll(".card.mt-2")).map(card => {
                            ...
                            const id = adt[0] || '';
                            const icon = a?.children[0] ? a.children[0].src : '';
    ...
    async fetchClientUserBookmarkAtIndex(index) {
        ...
        const response = await page.evaluate((index) => {
            ...
            const a = elements[0].querySelector(".thumbnail");
            const adt = a ? a.href.split("/").pop()?.split(".") : [];
            const id = adt ? adt[0] : '';
            const icon = a ? a.children[0].src : '';
            const bdt = elements[1].children;
            const nameElement = bdt[0].querySelector("i");
            const name = nameElement ? nameElement.innerText : '';
            const ownerElement = bdt[0].querySelector("strong");
        ...
        for (let i = 0; i < amount; i++) {
            const id = await page.evaluate(() => {
                ...
                if (!editBookmarkButton)
                    throw new Error("Edit bookmark button not found");
